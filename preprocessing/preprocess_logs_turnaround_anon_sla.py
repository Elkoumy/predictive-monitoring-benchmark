...
in_filename = "turnaround_anon_sla.csv"
...
dynamic_cat_cols = ["activity",'WIP']  # i.e. event attributes
static_cat_cols = ['SLA MIN','duration']  # i.e. case attributes that are known from the start
dynamic_num_cols = ['WIP']
static_num_cols = ['duration']
...
tmp = data.groupby(case_id_col).apply(check_if_any_of_activities_exist,
...
# first labeling
dt_labeled = data.sort_values(timestamp_col, ascending=True, kind="mergesort").groupby(case_id_col).apply(
    check_if_activity_exists_and_time_less_than, activity="Return ER")
